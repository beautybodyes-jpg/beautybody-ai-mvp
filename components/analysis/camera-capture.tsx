...
export function CameraCapture({ onCapture }: CameraCaptureProps) {
  ...
  const handleConfirm = useCallback(() => {
    if (previewUrl && capturedFile) onCapture(previewUrl, capturedFile);
        ...
        {mode === "select" && (
          <motion.div key="select" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-4">
            <GlassCard className="p-8 text-center">
              ...
              <h3 className="text-xl font-display text-white mb-2">{t("scan.captureLabel")}</h3>
              <p className="text-white/50 text-sm mb-7 leading-relaxed">{t("scan.captureDesc")}</p>
              <div className="space-y-3">
                <Button onClick={startCamera} className="w-full" glow size="lg">
                  <Camera className="mr-2 h-5 w-5" />
                  {t("scan.openCamera")}
                </Button>
                <Button variant="outline" className="w-full" onClick={() => fileInputRef.current?.click()} size="lg" isLoading={isUploading}>
              ...
              {error && (
                <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="mt-4 flex items-center gap-2 text-score-priority text-sm p-3 rounded-xl bg-score-priority/5 border border-score-priority/10">
            ...
            <div className="relative rounded-2xl overflow-hidden bg-black aspect-[3/4]">
              ...
              <div className="absolute inset-0 pointer-events-none">
                ...
                <div className="absolute top-4 left-4 right-4 flex justify-between">
                  <span className="text-white/60 text-xs bg-black/40 px-3 py-1 rounded-full backdrop-blur-sm">{t("scan.frontCamera")}</span>
                </div>
                <div className="absolute bottom-4 left-0 right-0 text-center">
                  <span className="text-white/70 text-sm bg-black/50 px-4 py-2 rounded-full backdrop-blur-sm">{t("scan.positionFace")}</span>
                </div>
              </div>
            </div>
            <div className="flex gap-3 mt-4">
              <Button variant="secondary" className="flex-1" size="lg" onClick={() => { stopCamera(); setMode("select"); }}>
          ...
          <motion.div key="preview" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
            <div className="relative rounded-2xl overflow-hidden aspect-[3/4] bg-black">
              ...
              <div className="absolute top-4 right-4">
                <span className="text-white/80 text-xs bg-black/50 px-3 py-1 rounded-full backdrop-blur-sm">{t("scan.previewLabel")}</span>
              </div>
            </div>
            <div className="flex gap-3 mt-4">
