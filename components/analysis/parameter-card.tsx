...
export function ParameterCard({ parameter, index, isPriority, isStrength }: ParameterCardProps) {
  ...
  const label = getSeverityLabel(parameter.score);
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-white font-medium text-sm truncate">{parameter.name}</h4>
          ...
          <ChevronDown
            className={cn(
              "w-5 h-5 text-white/30 shrink-0 transition-transform duration-300",
          ...
          {expanded && (
            ...
            >
              <div className="pt-4 mt-4 border-t border-white/[0.06]">
                <p className="text-white/50 text-sm leading-relaxed">{parameter.description}</p>
                <div className="mt-3 p-3 rounded-xl bg-white/[0.02] border border-white/[0.04]">
                  <p className="text-white/40 text-xs leading-relaxed">
                    <span className="text-champagne-400/70 font-medium">Tip: </span>
                    {parameter.tip}
                  </p>
                </div>
              </div>
